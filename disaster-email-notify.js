...
(function (global) {
  ...
  function openMailtoLink(to, sub, body) {
    var href = 'mailto:' + to + '?subject=' + encodeURIComponent(sub) + '&body=' + encodeURIComponent(body);
  ...
  global.disasterOpenMailtoReturned = function (rec) {
    var to = safeMailtoAddr(rec && rec.reporter_notify_email);
    if (!to) return;
  ...
  global.disasterTryNotifyWorkflow = function (kind, rec) {
    ...
    if (kind === 'submitted') {
      ...
      to = (rec.reporter_notify_email || '').trim();
      if (!to) return;
    } else {
      return;
    }
