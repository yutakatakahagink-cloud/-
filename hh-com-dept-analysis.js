...
(function(global){
  ...
  function buildDeptBlock(a,compact){
    if(!a.dataCount){
      if(compact){
        return '<div style="margin-bottom:8px;padding:8px 10px;border-left:3px solid '+a.color+';background:rgba(37,99,235,.04);border-radius:var(--rs)"><div style="font-size:12px;font-weight:700;color:'+a.color+'">'+esc(a.name)+'</div><div style="font-size:10px;color:var(--t3);margin-top:4px">報告'+a.total+'件（背景要因・職場環境データなし）</div></div>';
      }
    ...
    if(compact){
      return '<div style="margin-bottom:8px;padding:8px 10px;border-left:3px solid '+a.color+';background:rgba(37,99,235,.04);border-radius:var(--rs)"><div style="font-size:12px;font-weight:700;color:'+a.color+'">'+esc(a.name)+' <span style="font-weight:500;color:var(--t2)">（分析'+a.dataCount+'件）</span></div><div style="font-size:10px;color:var(--t2);margin-top:4px;line-height:1.55">'+stressLevelText(a.stAll).replace(/<\/?strong[^>]*>/g,'')+' ／ '+workplaceLevelText(a.weAll).replace(/<\/?strong[^>]*>/g,'')+'</div></div>';
    }
