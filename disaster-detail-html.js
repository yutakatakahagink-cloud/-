...
(function (global) {
  ...
  global.disasterBuildDetailHtml = function (r) {
    ...
    h += row('工事件名', V(r.keigen || ''));
    h += row('災害日時', V(r.datetime || ''));
    h += row('災害場所', V(r.basho || r.place || ''));
    h += row('住所', V(r.basho_jusho || ''));
    h += gs('被災者又は<br>事故者');
    h += sub('住所', V(r.jusho || ''));
    h += sub(
      '氏名',
    ...
    h += ge;
    h += gs('被災(事故)の<br>程度');
    h += sub('傷病名<br>(損害状況)', V(r.shobyomei || ''));
    ...
    h += sub('(人)', V(r.kyukun_person || ''));
    h += sub('(設備)', V(r.kyukun_equip || ''));
    h += sub('(管理)', V(r.kyukun_mgmt || ''));
    h += ge;
    ...
    h += ss('●場所(・・・で)', r.basho_detail || '');
    h += ss('●作業(・・・しているとき)', r.sagyo || '');
    ...
    if (r.wf && Array.isArray(r.wf.report_addenda) && r.wf.report_addenda.length) {
      h +=
        '<div style="margin-top:10px;border-top:2px solid #E65100;padding:10px 12px;background:#FFFDE7"><div style="font-weight:bold;font-size:11pt;margin-bottom:10px;color:#E65100;border-bottom:1px solid #FFCC80;padding-bottom:6px">追記・訂正（ワークフロー）</div>';
      r.wf.report_addenda.forEach(function (e) {
        ...
        h +=
          ...
          V(e.role || '追記') +
          '</strong>　<span style="color:#333">' +
          V(e.by || '') +
          '</span>　<time style="color:#888">' +
          V(at) +
          '</time></div>' +
